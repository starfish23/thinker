...
Template.editor.events =
  ...
  "click .save": (e) ->
    e.preventDefault()
    id = Session.get("document")
    result = Meteor.call "getDocumentText" 
    alert(result)
...
Template.userssss.name = ->
  currentUser = Meteor.userId()
  name = []
  users = Meteor.users.findOne({})
  name.push(users.emails[0].address) 
  return name
    
Template.docListss.documents = ->
  users = [Meteor.userId()]
  Documents.find({$or: [{userId: users[0]}, {invitedUsers: {$in: users}}]})

Template.docListss.events =
  "click button": ->
    

Template.docItemss.current = ->
  Session.equals("document", @_id)

Template.docItemss.events =
  "click a": (e) ->
    e.preventDefault()
    Session.set("document", @_id)
    Session.set("owner", @userId)
